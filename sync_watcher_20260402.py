...
def save_gdf_direct(gdf, table_name, schema, project_path):
    """
    [데이터 물리 저장 로직 - STT 기능 추가]
    record 컬럼 감지 시 audio_txt 컬럼을 동적으로 추가하고 STT를 수행합니다.
    """
    conn = get_pg_conn()
    ...
    try:
        ...
        # 1. 컬럼 리스트 구성 (record 뒤에 audio_txt 강제 삽입)
        final_cols = []
