...
def get_latest_job_id(project_id):
    ...
    try:
        ...
        err = str(e)
        # ✅ 404는 영구 스킵 대상으로 명확히 분리
        if "404" in err or "object_not_found" in err or "Not Found" in err:
            print(f"    ⚠️ [404] {project_id} → QFieldCloud에 없는 프로젝트")
            return "PROJECT_NOT_FOUND"
        if "401" in err or "Unauthorized" in err:
            client = login_client()
        # ✅ 그 외 오류는 JOB_CHECK_ERROR
